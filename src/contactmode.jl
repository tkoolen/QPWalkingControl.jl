struct ContactMode{T}
    ...
    active_points_body::Dict{BodyID, Vector{SPoint3D{T}}}
    active_points_frame::Dict{BodyID, Vector{SPoint3D{T}}}
...
function ContactMode{T}(frame::CartesianFrame3D, bodies) where T
    ...
    active_points_frame = Dict{BodyID, Vector{SPoint3D{T}}}()
    for body in bodies
        active_points_body[BodyID(body)] = SPoint3D{T}[]
        active_points_frame[BodyID(body)] = SPoint3D{T}[]
    end
    projected_points = SVector{2, T}[]
    hull = ConvexHull{CCW, T}()
    ContactMode{T}(frame, active_points_body, active_points_frame, projected_points, hull)
end
...
function update!(mode::ContactMode{T}, transformsource::Union{MechanismState, AbstractDict{BodyID, <:Transform3D}};
    ...
    for (bodyid, points_body) in mode.active_points_body
        points_frame = mode.active_points_frame[bodyid]
        ...
        @inbounds for point_body in points_body
            ...
            push!(mode.projected_points, horizontal_projection(points_frame[i].v))
            i += 1
        end
    end
